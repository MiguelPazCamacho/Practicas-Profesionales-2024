...
verticelu = np.array([-11,3.5])
verticeld = np.array([-11,-3.5])

verticeru = np.array([10.5, 3.5])
verticerd = np.array([10.5, -3.5])
...
# Generar puntos
x_values = np.arange(verticelu[0], verticerd[0] + eps, eps)
y_values = np.arange(verticeld[1], verticelu[1] + eps, eps)
...
#print(uno," ",lista_posiciones[0][0]," ",lista_posiciones[0][1]," ",dos)

#Ruta del archivo donde quieres guardar el URDF
file_path = 'Mundos/mundo_'
extension = ".urdf"

# Escribir la cadena en el archivo
for i in range(len(lista_posiciones)):
    num = str(i)
    loc_path = file_path+num+extension
    with open(loc_path, 'w') as file:
        file.write(uno+" "+str(lista_posiciones[i][0])+" "+str(lista_posiciones[i][1])+" "+dos)

    print(f"URDF guardado en {loc_path}")